...
type OkData<O extends Ok<D> | D, D extends unknown = unknown> = O extends { ok: boolean, data: D } ? OkData<O['data']> : O;

export function ok(): Ok<null>;
export function ok(value: undefined): Ok<undefined>;
export function ok<T extends Ok<D>, D extends unknown, ID extends OkData<T>>(data: T): Ok<ID>;
...
export function err<N extends number>(errMessage: string, errCode: N): Err<N>;
export function err<X extends {[key: string]: unknown}>(errMessage: string, errCode: number, errContext: X): Err<number, Error, X>;
export function err<E extends Error, N extends number, X extends {[key: string]: unknown}>(e: E, errMessage?: string, errCode?: N, errContext?: X): Err<N, E, X>;
export function err<N extends number, E extends Error, X extends {[key: string]: unknown}>(err: { errMessage?: string; errCode?: N; errContext?: X, errException?: E }): Err<N, E, X>;
	...
	if (a !== null && a !== undefined) {
		if (isResult(a)) {
			...
			switch (typeof a) {
				...
				case "object":
					if (a instanceof Error) { // err(Error, message?, code?, context?)
						exception = a;
						message = typeof b === "string" ? b : (a.message || "");
						code = typeof c === "number" ? c : 0;
						context = typeof d === "object" ? d as {[key: string]: unknown} : null;
					}
					else {
...
export function isOkResult<D extends unknown = unknown>(value: unknown): value is Ok<D> {
...
export function isResult(value: unknown): value is Result {
	if (typeof value === 'object' && value !== null && "ok" in value && "err" in value) {
...
export function from<D extends unknown, R extends Result<D>[]>(results: R): Ok<D[]> | Err<number, Error, { results: R }> {
	if (allOk(results)) {
